-- A scanned customer account is attached to every POS order type.
-- Loyalty visits and rewards remain limited to dine-in POS orders.
...
declare
  ...
  new_order_id bigint; receipt text; customer uuid; customer_display_name text;
  visit_count bigint; visit_number integer;
begin
  ...
  if loyalty_scan_code is not null then
    select id, full_name into customer, customer_display_name
      from public.profiles where loyalty_code = loyalty_scan_code and role = 'customer' for update;
    if customer is null then raise exception 'Customer QR was not found'; end if;

    if sale_order_type = 'dine_in' then
  ...
  values (coalesce(nullif(trim(customer_display_name), ''), 'Walk-in customer'), '', '', sale_order_type, 'confirmed', computed_subtotal, nullif(trim(sale_notes), ''),
  ...
  if customer is not null and sale_order_type = 'dine_in' then
  ...
  return jsonb_build_object('order_id', new_order_id, 'receipt_number', receipt,
    ...
    'change', computed_change, 'customer_id', customer);
end;
$$;
